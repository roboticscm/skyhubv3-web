<script>
  ...
  const applyTable = () => {
    ...
    getTableId().SelectableTable(
      ...
      function (obj) {
        ...
        startRow = document.querySelector(
          `#row_${obj.rows[0]}_Id :first-child`
        );
        if (startRow) {
          startRow.focus();
        }
        ...
        .map((it) => {
          for (let field in it) {
            if (!isHiddenColumn(field) && field !== "id" && typeof it[field] === "string") {
              const marked = StringUtil.markStringSearch(
                it[field],
                textSearch,
                true
              );
  ...
  const findRowById = (id) => {
    for (let i = 0; i < filteredData.length; i++) {
      if (
        id &&
        filteredData[i].id &&
        id.toString() === filteredData[i].id.toString()
      ) {
        return i;
      }
    }
  ...
  function checkKey(e) {
    ...
    if (e.code == "ArrowUp") {
      ...
      let nextrow =
        startRow.parentElement && startRow.parentElement.previousElementSibling;
      if (nextrow != null) {
      ...
      let idx = startRow.cellIndex;
      let nextrow =
        startRow.parentElement && startRow.parentElement.nextElementSibling;
      if (nextrow != null) {
        let sibling = nextrow.cells[idx];
        let row = Number(sibling.id.split("_")[1]);
        ...
        <tbody>
          {#each filteredData as row, rowIndex}
            <tr id={'row_' + rowIndex + '_Id'}>
              {#if showRowNumber}
                <th
                  id={`cell_${rowIndex}_${0}_${id}`}
                  class="freeze row-number">
                  {startRowCount + rowIndex}
                </th>
              {/if}
